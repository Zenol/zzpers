...
int main(int argc, char* argv[])
{
    ...
    // Change the first vertex of the point cloud
    try
    {
      if (first_vertex != 0)
	std::swap(points.at(0), points.at(first_vertex));
    }
    catch(std::exception&)
    {
      std::cerr << "First vertex out of range" << std::endl;
    }
                                         ...
                                         std::string&  outfilename)
{
    ...
    visible.add_options()
        ...
        ("first-vertex,i",      po::value<unsigned>(&first_vertex)->default_value(0),           "First vertex appairing in the filtration")
